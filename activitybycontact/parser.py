...
from genomic_functions import activity_of_element, coordinate_conversion, format_bedstyle, rescale_bin, get_contacts, promoter_coord, close_element, selfBin_contact, abc_score
...
# READING ARGUMENTS
    ...
    parser.add_argument('--bigwig', nargs='+', help='BigWig files for activity of element', type=str)
    ...
    parser.add_argument('--cutoff', type=int, help='Cutoff probability', default=0.5)
    ...
    bigwig_list = [pbw.open(file) for file in bigwig_files]
    tss_df = pd.read_csv(tss_bedfile, sep='\t')
    ...
    try:
        enhancer_df = enhancer_process(bigwig_data=bigwig_list, enhancer_info=enhancer_df,
                                       filtered_bincoord=filtered_bins, coordinate_liftover=lift_file,
        ...
        denom_dict, tss_enhancer_newinf = calculate_denominator(enhancer_tss_info=tss_enhancer_intersected,
                                                                ...
                                                                bigwig_data=bigwig_list, num_p=num_cores)

        #tss_enhancer_newinf = pd.read_csv('ehancer_tss_info.tsv', sep='\t')

        #with open('correction_values.json') as json_file:
        #    denom_dict = json.load(json_file)

        calculate_abc(enhancer_tss_info=tss_enhancer_newinf, denominator_values=denom_dict,
                      num_p=num_cores, positive_cutoff=CUTOFF_POSITIVE)
...
def enhancer_process(bigwig_data, enhancer_info, filtered_bincoord, coordinate_liftover, chromosome_relation):
    ...
    enhancer_info['activity'] = enhancer_info.apply(activity_of_element, axis=1,
                                                    args=('chromosome', 'start', 'end', bigwig_data),
                                                    result_type='expand')
    ...
    enhancer_info[['chromosome_conv', 'start_conv', 'end_conv']] = enhancer_info.apply(coordinate_conversion,
                                                                                          args=('chromosome',
                                                                                                'midpoint_start',
                                                                                                'midpoint_end',
    ...
    enhancer_imp = format_bedstyle(enhancer_info, name_col='Enhancer id', score_col='activity')
    ...
    enhancer_set = enhancer_info.loc[enhancer_info['Enhancer id'].isin(enh_bins_intersect_df['Enhancer id'].tolist())]
    enhancer_set.set_index('Enhancer id', inplace=True)
    enh_bins_intersect_df.set_index('Enhancer id', inplace=True)
    enhancer_processed = enhancer_set.join(enh_bins_intersect_df[['enhBin_start', 'enhBin_end', 'enhBin_id']])
...
def tss_process(tss_info, filtered_bincoord, coordinate_liftover, chromosome_relation):
    ...
    tss_info[['chromosome_conv', 'start_conv', 'end_conv']] = tss_info.apply(coordinate_conversion,
                                                                             args=('chromosome', 'start', 'end',
                                                                                   coordinate_liftover.convert_coordinate,
                                                                                   chromosome_relation),
                                                                             axis=1, result_type='expand')

    tss_imp = format_bedstyle(tss_info, name_col='Ensembl gene id', score_col='dot')
    ...
    tss_set = tss_info.loc[tss_info['Ensembl gene id'].isin(tss_bins_intersect_df['Ensembl gene id'].tolist())]
    tss_set.set_index('Ensembl gene id', inplace=True)
    tss_bins_intersect_df.set_index('Ensembl gene id', inplace=True)
    tss_processed = tss_set.join(tss_bins_intersect_df[['tssBin_start', 'tssBin_end', 'tssBin_id']])
    tss_processed.reset_index(inplace=True)

    print(tss_processed.head())
    num_bins = len(tss_processed['tssBin_id'].drop_duplicates().tolist())
...
def intersect_elements(tss_intersect, enhancer_intersect, closer_value):
    ...
    tss_bed_df = tss_intersect[['chromosome', 'start', 'end', 'Ensembl gene id', 'tssBin_id', 'strand']]

    enhancer_intersect['binID_activity'] = enhancer_intersect['enhBin_id'].astype(str) + ':' + enhancer_intersect['activity'].astype(str)
    enhancer_intersect['enhancer_midpoint'] = enhancer_intersect['Enhancer id'].astype(str) + ':' + enhancer_intersect['midpoint_start'].astype(str)
    enhancer_bed_df = enhancer_intersect[['chromosome', 'enhancer_start', 'enhancer_end',
    ...
    results_window = window_expanded[['tss_chrom', 'tss_start', 'tss_end', 'Ensembl gene id', 'tssBin_id', 'strand',
...
def calculate_denominator(enhancer_tss_info, scaled_counts, gamma_powerlaw, scale_powerlaw, s_value, distance_min,
                          promoter_length, bigwig_data, num_p):
    ...
    enhancer_tss_info[['promoter_start', 'promoter_end']] = enhancer_tss_info.apply(promoter_coord,
                                                                                    args=(promoter_length, 'tss_start'),
                                                                                    axis=1,
                                                                                    result_type='expand')

    enhancer_tss_info['promoter_activity'] = enhancer_tss_info.apply(activity_of_element, axis=1,
                                                                     args=('tss_chrom', 'promoter_start',
                                                                           'promoter_end', bigwig_data))

    gene_list = enhancer_tss_info['Ensembl gene id'].drop_duplicates().tolist()
...
def calculate_abc(enhancer_tss_info, denominator_values, num_p, positive_cutoff):
    ...
    abc_positives.to_csv('abc_test_results.tsv', sep='\t', index=False)
